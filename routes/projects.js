...
router.get('/:id', async (req,res,next)=>{
    try{
        ...
        if (project){
            ...
            res.json({message: `No projcet found ${req.params.id}`})
        }   
    }
...
router.put('/:id', async(req,res,next)=>{
    try{
        const {id}=req.params
        const {body}=req
        const updatedProject=await Project.findByIdAndUpdate(id, body, {new:true}) //get back the updated document after the update
        if(updatedProject){ 
...
router.post('/:id/tasks', async(req,res,next)=>{
    try{ //find the project to add a new task
        const project=await Project.findById(req.params.id)
        console.log(project)
        if (!project){
